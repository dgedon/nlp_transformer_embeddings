...
class Vocabulary:
    ...
    def __init__(self, max_voc_size=None, character=False, tokenizer_choice=None, stoi = None, itos=None):
        ...
        if self.character:
            ...
            if tokenizer_choice is None or tokenizer_choice == 'simple':
                ...
                self.autotokenizer = RobertaTokenizerFast.from_pretrained("roberta-base")
                self.tokenizer = lambda s: self.autotokenizer.tokenize(s)
    ...
    def encode_pretrain(self, docs):
        unkn_index = self.stoi[self.UNKNOWN]
        if self.character:
            encoded = [[self.stoi.get(w, unkn_index) for w in doc] for doc in docs]
        else:
            if self.autotokenizer is None:
                encoded = [self.stoi.get(w, unkn_index) for w in docs]
            else:
                enc = self.autotokenizer(docs)
                encoded = enc['input_ids']
                # encoded = [self.autotokenizer.tokenize(doc) for doc in docs]
